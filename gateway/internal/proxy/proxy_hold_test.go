...
func TestProxy_SettleReceipt(t *testing.T) {
	...
	if actualCost != "14014" {
		t.Errorf("Expected actualCost '14014' (from formula), got %q", actualCost)
	}
	if nonce != "789" {
	...
	// 10. 验证签名有效性
	expectedMsg := "0xChannel999:50000:14014:789"
...
func TestProxy_SettleReceipt_MockKey(t *testing.T) {
	...
	if actualCost != "5505" {
		t.Errorf("Expected actualCost '5505', got %q", actualCost)
	}
	if nonce != "999" {
	...
	expectedMsg := "0xChannelMock:20000:5505:999"
	expectedMsgHash := crypto.Keccak256Hash([]byte(fmt.Sprintf("\x19Ethereum Signed Message:\n%d%s", len(expectedMsg), expectedMsg)))
...
func TestProxy_MissingAgentCost(t *testing.T) {
	...
	actualCost := parts[2]
	if actualCost != "3003" {
		t.Errorf("Expected default actualCost to be '3003' when X-Agent-Cost is missing, got %q", actualCost)
	}
}
...
func TestProxy_SettleReceipt_NegativeCost(t *testing.T) {
	...
	actualCost := parts[2]
	if actualCost != "3003" {
		t.Errorf("Expected default actualCost to be '3003' when X-Agent-Cost is negative, got %q", actualCost)
	}
}
...
func TestProxy_SettleReceipt_MalformedCost(t *testing.T) {
	...
	actualCost := parts[2]
	if actualCost != "3003" {
		t.Errorf("Expected default actualCost to be '3003' when X-Agent-Cost is malformed, got %q", actualCost)
	}
}
